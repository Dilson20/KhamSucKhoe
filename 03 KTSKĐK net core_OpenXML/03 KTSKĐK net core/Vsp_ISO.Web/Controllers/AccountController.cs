        ...
        public async Task<IActionResult> Login(LoginViewModel login)
        {
            ...
            if (userfindByCore != null)
            {
                ...
                if (passwordCheck)
                {
                    ...
                    // Load existing roles
                    var roles = await _userManager.GetRolesAsync(userfindByCore);
                    System.Diagnostics.Debug.WriteLine($"[LOGIN] User: {login.DanhSo}, Roles found: {roles.Count}");
                    foreach (var role in roles)
                    {
                        System.Diagnostics.Debug.WriteLine($"[LOGIN] Adding role: {role}");
                        cookieClaims.Add(new Claim(ClaimTypes.Role, role));
                    }
                    ...
                    // Return roles in response for debugging
                    return Json(new { success = true, redirectUrl = Url.Action("MainPage", "Home"), rolesCount = roles.Count, roles = roles });
                }
                else
                {
                    return Json(new { success = false, error = "Invalid username or password." });
            ...
            else
            {
                ...
                // Load existing roles for legacy users as well
                var roles = await _userManager.GetRolesAsync(appUser);
                System.Diagnostics.Debug.WriteLine($"[LOGIN-LEGACY] User: {login.DanhSo}, Roles found: {roles.Count}");
                foreach (var role in roles)
                {
                    System.Diagnostics.Debug.WriteLine($"[LOGIN-LEGACY] Adding role: {role}");
                    cookieClaims.Add(new Claim(ClaimTypes.Role, role));
                }
